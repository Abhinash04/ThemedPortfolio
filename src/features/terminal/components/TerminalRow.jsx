...
const renderResultElement = (element, rowKey) => {
  ...
  if (typeof element === "string") {
    const mapped = renderTerminalContent(element);
    return mapped !== undefined ? mapped : <span>{element}</span>;
  }
  ...
  if (Array.isArray(element) && element.every(item => React.isValidElement(item) || typeof item === "string")) {
    return <span>{element}</span>;
  }
