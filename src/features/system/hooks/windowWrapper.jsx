...
const WINDOW_DEFAULT_POSITIONS = {
  terminal: { top: "10%", left: "12%" },
  safari:   { top: "12%", left: "18%" },
  finder:   { top: "8%",  left: "8%"  },
  contact:  { top: "15%", left: "28%" },
  resume:   { top: "6%",  left: "22%" },
  skills:   { top: "14%", left: "16%" },
  ...
  const Wrapped = (props) => {
    ...
    useGSAP(() => {
      ...
      if (
        ...
        const defaultPos = WINDOW_DEFAULT_POSITIONS[windowKey] ?? { top: "15%", left: "20%" };
        const prevTransform = manageGeometryDataset.getPrev("prevTransform", node) ?? "";
        gsap.to(node, {
          scale: 1,
          opacity: 1,
          top: manageGeometryDataset.getPrev("prevTop", node) ?? defaultPos.top,
          left: manageGeometryDataset.getPrev("prevLeft", node) ?? defaultPos.left,
